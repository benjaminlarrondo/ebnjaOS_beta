...
- Supabase config now reads `SUPABASE_SCHEME`, `SUPABASE_HOST`, `SUPABASE_PATH`, and `SUPABASE_ANON_KEY` from build configuration instead of hardcoded Swift values.
...
## Notes
- `fitness_body_metrics` and `fitness_workouts` now use deterministic UUID ids derived from user + external id so REST upserts remain idempotent even when unique indexes are not available through the anon client.
