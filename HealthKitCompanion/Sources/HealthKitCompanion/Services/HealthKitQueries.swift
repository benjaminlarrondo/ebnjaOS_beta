...
@MainActor
final class HealthKitQueries: ObservableObject {
    ...
    static func defaultReadTypes() -> Set<HKObjectType> {
        ...
        var objectTypes: [HKObjectType] = quantityTypes
        if let sleepType {
            objectTypes.append(sleepType)
        }
        objectTypes.append(workoutType)
        return Set(objectTypes)
    }
